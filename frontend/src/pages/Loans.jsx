            ...
            <table class="w-full text-left border-collapse">
              <thead>
                <tr class="bg-slate-900/80 text-slate-400 border-b border-slate-800 text-xs font-semibold uppercase tracking-wider">
                  ...
                  {currentUser.role !== 'MEMBER' && <th class="py-4 px-6">Borrower</th>}
                  <th class="py-4 px-6">Date Borrowed</th>
                  <th class="py-4 px-6">Due Date</th>
                  <th class="py-4 px-6">Returned Date</th>
              ...
              <tbody class="divide-y divide-slate-800/50 text-sm text-slate-300">
                {loans.map((loan) => (
                  <tr key={loan.id} class="hover:bg-slate-900/30 transition-colors">
                    <td class="py-4 px-6 font-semibold text-slate-100">{loan.bookTitle}</td>
                    <td class="py-4 px-6 font-mono text-xs">{loan.bookIsbn}</td>
                    {currentUser.role !== 'MEMBER' && (
                      <td class="py-4 px-6 text-slate-200">
                        <span class="font-medium">{loan.username}</span>
                      </td>
                    )}
                    <td class="py-4 px-6 font-mono text-xs">{loan.borrowDate}</td>
                    <td class="py-4 px-6 font-mono text-xs text-indigo-300">{loan.dueDate}</td>
                    ...
                    <td class="py-4 px-6">{getStatusBadge(loan.status)}</td>
